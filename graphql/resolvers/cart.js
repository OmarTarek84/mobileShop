...
module.exports = {
    addToCart: (args, req) => {
        ...
        return Cart.findOne({mobileId: selectedMobile._id, userId: req.userId}).then(cart => {
            if (cart || cart != null) {
    ...
    removeCart: (args, req) => {
        ...
        return Cart.findById(cartId).populate('mobileId').then(cart => {
            return Cart.deleteOne({_id: cartId}).then(res => {
                return User.findById(req.userId).then(user => {
                    user.cart.pull(cart);
                    return user.save().then(result => {
                        return {
    ...
    cart: (args, req) => {
        ...
        return Cart.find({userId: userId}).then(carts => {
    ...
    createOrder: (args, req) => {
            ...
            return Cart.find({userId: req.userId}).populate('mobileId').then(carts => {
                return carts.map(cart => {
                ...
                return order.save().then(res => {
                        return User.findById(req.userId).then(user => {
                            ...
                            return user.save().then(resData => {
                                return Cart.deleteMany({userId: req.userId}).then(c => {
                                    return {
                                        ...res._doc,
                                        _id: res._doc._id.toString(),
                                        userId: singleUser.bind(this, res._doc.userId),
                                        createdAt: new Date(res._doc.createdAt).toLocaleDateString(),
                                        updatedAt: new Date(res._doc.updatedAt).toLocaleDateString()
                                    }
                                })
                                .catch(err => {
